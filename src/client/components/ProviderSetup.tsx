...
export function ProviderSetup({ controller }: { controller: Controller }) {
  ...
  return <>
    <div className="key-row">
      ...
      {usesKey && <>
        <label id="apiKeyLabel" htmlFor="apiKey">{providerConfig.credentialLabel}</label>
...
function OpenRouterClonePanel({ controller }: { controller: Controller }) {
  ...
  return <FieldPanel id="openrouterVoiceClonePanel" className="voice-clone-panel" live>
    <div className="voice-clone-header"><div><strong>Zero-shot voice clones</strong><span>Loaded {state.openrouterClones.length.toLocaleString()} locally saved voice clone{state.openrouterClones.length === 1 ? "" : "s"}.</span></div><Button type="button" onClick={() => undefined}>Refresh voices</Button></div>
    <div className="voice-clone-form">
      ...
      <Button type="button" className="primary" disabled={state.operationBusy} onClick={() => void actions.saveOpenRouterClone({ name, languages, gender, file })}>{selected ? "Update voice clone" : "Create voice clone"}</Button>
      <Button type="button" disabled={!selected || state.operationBusy} onClick={actions.deleteOpenRouterClone}>Delete selected clone</Button>
    </div>
...
function MiniMaxSetup({ controller }: { controller: Controller }) {
  ...
  return <>
    ...
    <FieldPanel id="minimaxVoiceClonePanel" className="voice-clone-panel" live>
      ...
      <div className="voice-clone-form">
        ...
        <Button type="button" className="primary" disabled={state.operationBusy} onClick={() => void actions.saveMinimaxClone({ name, previewText, languageModel, promptText, validationText, source, prompt })}>Create voice clone</Button>
        <Button type="button" disabled={!selected || state.operationBusy} onClick={() => void actions.deleteMinimaxClone()}>Delete selected voice</Button>
      </div>
